...
{
	if( is_array( $msg ) ){
		http_response_code( ( isset( $msg['code'] ) ? $msg['code'] : 200 ) );
		die( json_encode( $msg ) );
	}else{
...
function sendMail( $settings = array() )
{
	...
	$mail->From 			= $SYSCONFIG['SMTP']['sender_addr']; 		// SENDER ADDRESS
	...
	if ( !$mail->send() ) {
		$dataSet["hasError"] = true;
		$dataSet["msg"] = $mail->ErrorInfo;
	} else {
		$dataSet["hasError"] = false;
	}
...
function trackEvent( $source = 'default', $mobile = 0 ){
