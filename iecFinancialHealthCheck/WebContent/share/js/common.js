...
page[1] = ["ajax/income_and_expense_animate.jsp", "#ajax"];//2
page[2] = ["ajax/loanmanage_animate.jsp", "#ajax"];//3
page[3] = ["ajax/financialgoal_animate.jsp", "#ajax"];
page[4] = ["ajax/investment_animate.jsp", "#ajax"];//5
...
function loadanimation(num) {
	...
	if (!nua.match(/(iPad|iPhone|android|BlackBerry|Nokia)/i)) {
		...
		if (num == 5) {
			$('.main').animate({
				backgroundColor: "#1a7db9"
			}, 'fast');
		} else {
			$('.main').animate({
				backgroundColor: "#AFE2D4"
			}, 'slow');
		}
	}
}
...
$(document).ready(function () {
	...
	$('.step_check input:radio[name=track0]').css({
		'display': 'block',
		'cursor': 'pointer'
	});
	//mouseover function
	...
	$('.trackstep').click(function () {
		...
		if (c != '' || c != 'undefined') {
			...
			if ($('.' + match[currentSlider - 1] + ' .successresult').hasClass('allowNext') || currentSlider == 1) {
				popup(currentNum);
				setTimeout(function () {
					$("span.sTitle").text($("div#track" + pagenum + " > .ele" + pagenum + "").text());
				}, 1000); //change page title
			} else
				notcompleteReminder();
		}
		//}
	...
	$(".silderBtn").click(function () {
		...
		if (id == 'minusBtn1' || id == 'addBtn1') {
			if ($('.income_and_expense input:radio[name=loan_radio]:checked').length == 0) {
				...
				$("#mount2").parents('.qAnsContainer').css('opacity', '0.5');
				$('.income_and_expense .flag').eq(2).find('.qTitle').addClass('dimblue');
				$('.income_and_expense .flag').eq(2).find('.qTitle p').css({
					'background-color': '#888',
					'color': '#c6c6c6'
				});
				$('.income_and_expense .flag').eq(2).find('.qTitle .qTitleImg').removeClass('qTitleImg').addClass('qTitledimImg');
	...
	$(".actionBtn").click(function () {
		...
		}, 400); //allowed time to slide down can help scroll correctly				
	...
	$("#mmenuTitle").click(function (e, p) {
		...
		if ($("#sectionTab").css("display") == 'block') {
			...
			$("body, html").animate({
				scrollTop: $("#headCaption").offset().top
			}, 10);
			keepMenuTitle($("#mmenuTitle"));
		}
...
function prevNextSlide(uID, num, slider) {
	...
	num--;
	setTimeout(function () {
		$("span.sTitle").text($("div#track" + num + " > .ele" + num + "").text());
	}, 1000);
}
...
function isasked(ele) {
	...
	if (exsist != 0) //if next question exists
	{
		...
		nextquestion.find('.qTitle p').css({
...
function changeAndSlide(num, radiobtn, inputradio) {
	...
	$('.isureBtn,.icancelBtn').click(function () {
		...
		if (btnId == 'isureBtn') {
			...
			setTimeout(function () {
				$("span.sTitle").text($("div#track" + pagenumber + " > .ele" + pagenumber + "").text());
			}, 1000); //change page title
...
function fbShare() {
	var targetLink = 'https://www.thechinfamily.hk/tools/fhc/tc/main/index.html'
	window.open('https://www.facebook.com/sharer/sharer.php?u=' + targetLink.replace('.jsp' , '.html'), 'sharer', 'width=626,height=436');
}
...
function mailShare() {
	...
	if (currentURL.search('/en/') !== -1) {
		...
		mailTo = "mailto:info@thechinfamily.hk?";
		body_line = escape("\n");
		mailSubject = "subject=個人理財分析";

		body = "&body=審視你的財務狀況，參考免費分析報告及具體行動方案助你改善理財。" +
			body_line + encodeURIComponent(targetLink);
		mailTo += mailSubject + body;
		$(".email").attr('href', mailTo);
	};
...
function feedBack() {
	var mail = 'mailto:info@thechinfamily.hk?',
...
function init_mMenuTitle(type) {
	if (type == "block") {
		...
		if (currentSlider == 10) {
			isExist = $("#mmenuTitle span#sTitle").length;
			sectionName = $("#sectionTab li.tabActive a").attr("class").split(" ")[0];
			...
			if (isExist == 0) {
				...
				$("#mmenuTitle #sTitle").attr("class", sectionName).text(textName);
		}
	}

}

function keepMenuTitle(menu) {
	var imagePath = menu.find('img').attr('src');
	var start = imagePath.indexOf('m_');
	if (imagePath.substring(start) == 'm_down.png') // when click the title here,image become down and the title is empty
	{
		sectionName = $("#sectionTab li.tabActive a").attr("class").split(" ")[0];
		textName = $("#sectionTab li.tabActive").text();
		$("#mmenuTitle span#sTitle").attr("class", sectionName).text(textName);
	}
}
